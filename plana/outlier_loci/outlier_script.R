...
filename="plana_outlier_out.pcadapt.zscores.npy"
...
p<-read.table("plana_Sites4PCAngsd.sites",colC=c("factor","integer"))
dflist=list()
for (i in 1:PCs){
  df=cbind(p,pval[,i])
...
plana_outliers = dflist



write.table(plana_outliers, "p_outliers.txt", sep = "\t", row.names = F)
