...
expDesign = read.csv("../../../expDesign.csv") %>%
  ...
  mutate(Site = factor(Site, levels = c("Kettle Cove", "Beverly", "Newport", "Cape May")))

write.csv(expDesign, "p_expDesign.csv")

expDesign = read.csv("p_expDesign.csv") %>% 
  mutate(Site = factor(Site, levels = c("Kettle Cove", "Beverly", "Newport", "Cape May")))

# covariance matric
co = as.matrix(read.table("../plana_outlier.covMat")) 
...
ggsave(plot = last_plot(), "plana_outlier_pca.pdf")
ggsave(plot = last_plot(), "plana_outlierl_pca.png")


## IBS

ma = as.matrix(read.table("../plana_outlier.ibsMat"))
